...
SELECT  descsegmproduto as "descrição do produto", count(*) as "total"
FROM produto as p
INNER JOIN segmento as s ON s. idsegmproduto = p.segmento
GROUP BY s.idsegmproduto;

SELECT  descsegmproduto as "descrição do produto", count(*) as "total"
FROM produto
INNER JOIN segmento ON idsegmproduto = produto.segmento
GROUP BY segmento.idsegmproduto;

/*
Qual a quantidade de produtos vendidos por Volume e Unidade de Medida? 
Especifique a quantidade de produtos em cada unidade de medida e volume. 
*/
SELECT  valorvolume, descrunidmedida, count(*) as "total"
FROM produto as p
INNER JOIN volume as v ON v.idvolume = p.volume
INNER JOIN unidmedida as u ON u.idunidmedida = p.volume
GROUP BY v.idvolume, u.idunidmedida;

/*Qual a quantidade de produtos do segmento de Limpeza vendidos por Unidade de Medida e Volume? */

select s.descsegmproduto, u.descrunidmedida, v.valorvolume, count(*) AS 'Qtde'
from produto as p
inner join segmento as s on s.idsegmproduto = p.segmento
inner join volume as v on v.idvolume = p.volume
inner join unidmedida as u on u.idunidmedida = p.unidademedida
where s.descsegmproduto = 'Limpeza'
group by s.descsegmproduto, u.descrunidmedida, v.valorvolume;
...
/*Qual a quantidade de produtos por Segmento? */

select segmento.descsegmproduto, count(*) as 'Qtde'
from produto as p
inner join segmento on segmento.idsegmproduto = p.segmento
group by segmento.descsegmproduto;
...
inner join volume v on v.idvolume = p.volume
inner join unidmedida u on u.idunidmedida = p.unidademedida
where s.descsegmproduto = 'Limpeza'
group by s.descsegmproduto, u.descrunidmedida, v.valorvolume;

/* Qual o produto mais barato do segmento de Mercearia vendido em Gramas?*/

select s.descsegmproduto, u.descrunidmedida, p.descproduto, p.valorunitario
from produto as p
inner join segmento as s on s.idsegmproduto = p.segmento
inner join unidmedida as u on u.idunidmedida = p.unidademedida
where s.descsegmproduto = 'Mercearia'
and u.descrunidmedida = 'Gramas'
and p.valorunitario = (
	select min(p.valorunitario)
	from produto as p
	inner join segmento as s on s.idsegmproduto = p.segmento
	inner join unidmedida as u on u.idunidmedida = p.unidademedida
	where s.descsegmproduto = 'Mercearia'
